		...
		var self   = this,
			...
			filter = function(file, asMap, type) { 
				var res = asMap? file : true,
					...
					vid, tarconfig_items, isRoot, rootNames;
					...
					if (isRoot || file.mime === 'directory') {
						...
						if (file.volumeid) {
							...
							if (isRoot) {
								...
								tarconfig_items = self.volOptions[vid];
								
								if (file.options) {
									// >= v.2.1.14 has file.options
									Object.assign(tarconfig_items, file.options);
								}
								
								// for compat <= v2.1.13
								if (file.disabled) {
									tarconfig_items.disabled = file.disabled;
									tarconfig_items.disabledFlip = self.arrayFlip(file.disabled, true);
								}
								if (file.tmbUrl) {
									tarconfig_items.tmbUrl = file.tmbUrl;
								}
								
								// '/' required at the end of url
								if (tarconfig_items.url && tarconfig_items.url.substr(-1) !== '/') {
									tarconfig_items.url += '/';
								}

								// check uiCmdMap
								chkCmdMap(tarconfig_items);
								
								// check trash bin hash
								if (tarconfig_items.trashHash) {
									if (self.trashes[tarconfig_items.trashHash] === false) {
										delete tarconfig_items.trashHash;
									} else {
										self.trashes[tarconfig_items.trashHash] = file.hash;
									}
								}
								
								// set immediate properties
								$.each(self.optionProperties, function(k) {
									if (tarconfig_items[k]) {
										file[k] = tarconfig_items[k];
									}
								});
