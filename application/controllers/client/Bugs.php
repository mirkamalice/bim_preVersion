...
class Bugs extends Client_Controller
{
    ...
    public function bugsList($type = null)
    {
        if ($this->input->is_ajax_request()) {
            ...
            $main_column = array('issue_no', 'bug_title', 'priority', 'bug_status');
...
// Update into tbl_project
    ...
    public function save_comments()
    {
        ...
        $data['module_field_id'] = $this->input->post('bug_id', TRUE);
...
//        //get all comments info
            ...
            $bwhere = array('module' => 'bugs', 'module_field_id' => $id);
            $this->bugs_model->_table_name = "tbl_attachments"; //table name
            $this->bugs_model->_order_by = "module_field_id";
            $data['files_info'] = $this->bugs_model->get_by($bwhere, FALSE);
    
            foreach ($data['files_info'] as $key => $v_files) {
                $this->bugs_model->_table_name = "tbl_attachments_files"; //table name
                $this->bugs_model->_order_by = "attachments_id";
                $data['project_files_info'][$key] = $this->bugs_model->get_by(array('attachments_id' => $v_files->attachments_id), FALSE);
            }
            $data['dropzone'] = true;
