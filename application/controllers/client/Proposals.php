...
class Proposals extends Client_Controller
{
    ...
    function send_email_proposals($proposals_id, $message, $subject)
    {
        ...
        $params['resourceed_file'] = 'uploads/' . slug_it(lang('proposal') . '_' . $proposals_info->reference_no) . '.pdf';
        $params['resourcement_url'] = base_url() . 'uploads/' . slug_it(lang('proposal') . '_' . $proposals_info->reference_no) . '.pdf';

        $this->attach_pdf($proposals_id);

        $this->proposal_model->send_email($params);
        //Delete estimate in tmp folder
        if (is_file('uploads/' . slug_it(lang('proposal') . '_' . $proposals_info->reference_no) . '.pdf')) {
            unlink('uploads/' . slug_it(lang('proposal') . '_' . $proposals_info->reference_no) . '.pdf');
        }
    }
