...
class Projects extends Admin_Controller
{
    ...
    public function all_projects_state_report()
    {
        if (!$this->input->is_ajax_request()) {
            redirect('admin/dashboard');
        }
        $data = array();
        $pathonor_jonno['all_projects_state_report_div'] = $this->load->view("admin/projects/all_projects_state_report", $data, true);
    ...
    public function projectList($filterBy = null, $search_by = null)
    {
        if ($this->input->is_ajax_request()) {
            ...
            $this->datatables->join_table = array('tbl_client', 'tbl_customer_group');
            $this->datatables->join_where = array('tbl_project.client_id=tbl_client.client_id','tbl_project.category_id=tbl_customer_group.customer_group_id');
            $this->datatables->select = 'tbl_project.*,tbl_client.name,tbl_customer_group.customer_group';
            
            $custom_field = custom_form_table_search(4);
            $main_column = array('tbl_project.project_name', 'tbl_customer_group.customer_group', 'tbl_project.tags', 'tbl_client.name', 'tbl_project.end_date', 'tbl_project.permission', 'tbl_project.project_status');
            ...
            if (!empty($search_by)) {
                ...
                if ($search_by == 'by_category') {
                    $where = array('tbl_project.category_id' => $filterBy, 'project_status !=' => 'completed');
                }
                if ($search_by == 'by_staff') {
                    if ($filterBy == 'everyone') {
                        $where = array('tbl_project.permission' => 'all');
                    } else {
                        $where = $filterBy;
                    }
                }
            ...
            foreach ($fetch_data as $key => $v_project) {
                if (!empty($v_project)) {
                    ...
                    if (strtotime(date('Y-m-d')) > strtotime($v_project->end_date) && $progress < 100 && $v_project->project_status != 'cancel') {
                    ...
                    $sub_array[] = $name;
                    if (!empty($v_project->category_id)) {
                        $category = '<span class="tags">' . $v_project->customer_group . '</span>';
                    } else {
                        $category = '';
                    }
                    $sub_array[] = $category;
                    $sub_array[] = get_tags($v_project->tags, true);
       ...
       class="btn btn-xs btn-purple"><i class="fa fa-copy"></i></a>' . ' ';
                        $action .= btn_edit('admin/projects/create/' . $v_project->project_id) . ' ';
                    }
    ...
    function save_invoice($project_id = null)
    {
        ...
        if (!empty($created) || !empty($edited) && !empty($project_id)) {
            ...
            $update = $this->input->post('create', true);
            
            if (!empty($save_as_draft) || !empty($update)) {
                $data = $this->invoice_model->array_from_post(array('reference_no', 'client_id', 'project_id', 'warehouse_id', 'discount_type', 'discount_percent', 'user_id', 'adjustment', 'discount_total', 'show_quantity_as'));
                ...
                if (!empty($items_data)) {
                    ...
                    foreach ($items_data as $items) {
                        ...
                        if (!empty($items['items_id'])) {
                            ...
                            if (!empty($qty_calculation) && $qty_calculation == 'Yes') {
                                $this->check_existing_qty($items_id, $items['quantity'], $data['warehouse_id']);
                            }
                        }
                        $index++;
                    }
            ...
            set_message($type, $message);
            redirect('admin/invoice/invoice_details/' . $invoice_id);
        } else {
            if (empty($_SERVER['HTTP_REFERER'])) {
                redirect('admin/projects');
            } else {
    ...
    function check_existing_qty($items_id, $qty, $warehouseId)
    {
        ...
        if ($items_info->quantity != $qty) {
            if ($qty > $items_info->quantity) {
                ...
                if (!empty($items_info->saved_items_id)) {
                    $this->invoice_model->reduce_items($items_info->saved_items_id, $reduce_qty, $warehouseId);
                }
            }
            if ($qty < $items_info->quantity) {
                $return_qty = $items_info->quantity - $qty;
                if (!empty($items_info->saved_items_id)) {
                    $this->invoice_model->return_items($items_info->saved_items_id, $return_qty, $warehouseId);
                }
            }
        }
        return true;
    ...
    public function project_details($id, $action = NULL, $op_id = NULL)
    {
        ...
        $data['id'] = $id;
        $data['name'] = $data['project_details']->project_name;
        $data['module'] = 'projects';
        $data['all_tabs'] = projects_details_tabs($id);
    ...
    public function save_attachment_comments()
    {
        ...
        if (!empty($comment_id)) {
            ...
            $view_data['comment_details'] = $this->db->where(array('task_comment_id' => $comment_id))->order_by('comment_datetime', 'DESC')->get('tbl_task_comment')->result();
            $response_data = $this->load->view("admin/projects/comments_list", $view_data, true);
            echo json_encode(array("status" => 'success', "data" => $response_data, 'message' => lang('projects_comment_save')));
            exit();
            //            $type = "success";
            //            $message = lang('project_comment_save');
            //            set_message($type, $message);
    ...
    function notify_attchemnt_project($attachments_id)
    {
        $comment_info = $this->items_model->check_by(array('attachments_id' => $attachments_id), 'tbl_attachments');
        $project_info = $this->items_model->check_by(array('project_id' => $comment_info->project_id), 'tbl_project');
    ...
    public function delete_files($attachments_id)
    {
    ...
    public function save_milestones($milestones_id = NULL)
    {
        ...
        $activities = array(
            ...
            'activity' => $action,
            'icon' => 'fa-folder-open-o',
            'link' => 'admin/projects/project_details/' . $id . '/milestones',
        ...
        $message = $msg;
        set_message($type, $message);
        redirect('admin/projects/project_details/' . $data['project_id'] . '/' . 'milestones');
    }
...
//        $task_id[] = 2;
    ...
    public function delete_project($id, $bulk = null)
    {
        ...
        if (!empty($deleted) && !empty($can_delete)) {
            ...
            if (!empty($project_info)) {
                ...
                $pwhere = array('module' => 'projects', 'module_field_id' => $id);
                // deleted project comments with file
                $all_comments_info = $this->db->where($pwhere)->get('tbl_task_comment')->result();
                ...
                $this->items_model->_table_name = "tbl_task_comment"; // table name
                $this->items_model->delete_multiple($pwhere);
                
                // deleted project attachment with file
                $this->items_model->_table_name = "tbl_attachments"; //table name
                $this->items_model->_order_by = "module_field_id";
                $files_info = $this->items_model->get_by($pwhere, FALSE);
                if (!empty($files_info)) {
                ...
                if (!empty($project_tasks)) {
                    foreach ($project_tasks as $v_taks) {
                        $twhere = array('module' => 'tasks', 'module_field_id' => $v_taks->task_id);
                        $all_comments_info = $this->db->where($twhere)->get('tbl_task_comment')->result();
                        if (!empty($all_comments_info)) {
                            foreach ($all_comments_info as $comments_info) {
                                if (!empty($comments_info->comments_attachment)) {
                                    $attachment = json_decode($comments_info->comments_attachment);
                        ...
                        //delete data into table.
                        $this->items_model->_table_name = "tbl_task_comment"; // table name
                        $this->items_model->delete_multiple($twhere);
                        
                        $this->items_model->_table_name = "tbl_attachments"; //table name
                        $this->items_model->_order_by = "module_field_id";
                        $files_info = $this->items_model->get_by($twhere, FALSE);
                        if (!empty($files_info)) {
                ...
                if (!empty($project_bugs)) {
                    foreach ($project_bugs as $v_bugs) {
                        $bwhere = array('module' => 'bugs', 'module_field_id' => $v_bugs->bug_id);
                        $all_comments_info = $this->db->where($bwhere)->get('tbl_task_comment')->result();
                        if (!empty($all_comments_info)) {
                            foreach ($all_comments_info as $comments_info) {
                                if (!empty($comments_info->comments_attachment)) {
                                    $attachment = json_decode($comments_info->comments_attachment);
                        ...
                        $this->bugs_model->delete_multiple($bwhere);
                        
                        
                        $this->bugs_model->_table_name = "tbl_attachments"; //table name
                        $this->bugs_model->_order_by = "module_field_id";
                        $files_info = $this->bugs_model->get_by($bwhere, FALSE);
                        
                        foreach ($files_info as $b_v_files) {
                            $uploadFileinfo = $this->db->where('attachments_id', $b_v_files->attachments_id)->get('tbl_attachments_files')->result();
                            if (!empty($uploadFileinfo)) {
                                foreach ($uploadFileinfo as $Fileinfo) {
                                    remove_files($Fileinfo->file_name);
                                }
                            }
                            $this->bugs_model->_table_name = "tbl_attachments_files"; //table name
                            $this->bugs_model->delete_multiple(array('attachments_id' => $b_v_files->attachments_id));
                        }
                        //delete into table.
                        $this->bugs_model->_table_name = "tbl_attachments"; // table name
                        $this->bugs_model->delete_multiple($bwhere);
                        ...
                        if (!empty($bug_tasks)) {
                            foreach ($bug_tasks as $tasks_bugs) {
                                $twhere = array('module' => 'tasks', 'module_field_id' => $tasks_bugs->bug_id);
                                $all_comments_info = $this->db->where($twhere)->get('tbl_task_comment')->result();
                                if (!empty($all_comments_info)) {
                                    foreach ($all_comments_info as $comments_info) {
                                        if (!empty($comments_info->comments_attachment)) {
    ...
    public function save_project_notes($id)
    {
        ...
        set_message($type, $message);
        redirect('admin/projects/project_details/' . $id . '/' . 'notes');
    }
    ...
    function update_project_timer($id = NULL, $action = NULL)
    {
                ...
                foreach ($notifiedUsers as $users) {
                    if ($users != $this->session->userdata('user_id')) {
                        add_notification(array(
                            ...
                            'link' => 'admin/projects/project_details/' . $t_data['project_id'] . '/timesheet',
                            'value' => lang('project') . ' ' . $project_info->project_name,
                        ));
                    }
                }
        ...
        if ($action == 'delete_task_timmer') {
            ...
            $message = $msg;
            set_message($type, $message);
            redirect('admin/projects/project_details/' . $t_data['project_id'] . '/timesheet');
        }
    }
    ...
    function tasks_timer($status, $project_id, $inline = null)
    {
        ...
        if ($status == 'off') {
            ...
            if ($check_user == TRUE) {
                ...
                $activities = array(
                    ...
                    'icon' => 'fa-folder-open-o',
                    'link' => 'admin/projects/project_details/' . $project_id . '/timesheet',
                    ...
                    foreach ($notifiedUsers as $users) {
                        if ($users != $this->session->userdata('user_id')) {
                            add_notification(array(
                                ...
                                'link' => 'admin/projects/project_details/' . $project_info->project_id . '/timesheet',
        ...
        if (!empty($inline)) {
            ...
            set_message($type, $message);
            if (empty($_SERVER['HTTP_REFERER'])) {
                redirect('admin/projects/project_details/' . $project_info->project_id . '/timesheet');
            } else {
                redirect($_SERVER['HTTP_REFERER']);
            }
        }
