...
class Estimates extends Admin_Controller
{
    ...
    public function index($action = NULL, $id = NULL, $item_id = NULL)
    {
        ...
        if ($action == 'estimates_details') {
            redirect('admin/estimates/estimates_details/' . $id);
    ...
    final public function create($action = NULL, $id = NULL, $item_id = NULL)
    {
        $data['page'] = lang('sales');
        $data['sub_active'] = lang('estimates');
        if (!empty($item_id)) {
    ...
    public function estimates_details($id, $pdf = NULL)
    {
        $data['title'] = "Estimates"; //Page title
        $data['title'] = "Estimates"; //Page title
        $data['sales_info'] = join_data('tbl_estimates', '*', array('estimates_id' => $id), array('tbl_client' => 'tbl_client.client_id = tbl_estimates.client_id'));
        if (!empty($data['sales_info'])) {
            $data['sales_info']->ref_no = lang('estimates') . ' : ' . $data['sales_info']->reference_no;
            $data['sales_info']->start_date = lang('estimate_date') . ' : ' . display_date($data['sales_info']->estimate_date);
            $url = base_url('frontend/estimates/') . url_encode($id);
            ...
            if (strtotime($data['sales_info']->due_date) < strtotime(date('Y-m-d')) && $data['sales_info']->status == 'draft') {
                ...
                $data['sales_info']->overdue_days = lang('estimate_overdue') . ' ' . lang('by') . ' ' . $overdue_days . ' ' . lang('days');
            ...
            $data['sales_info']->status = lang('estimate_status') . ' :  <span class="label label-' . $label . '">' . lang($data['sales_info']->status) . '</span>';
            $data['sales_info']->custom_field = '';
            $show_custom_fields = custom_form_label(10, $data['sales_info']->estimates_id);
            ...
            $data['items_id'] = 'estimate_items_id';
            $data['all_items'] = $this->estimates_model->ordered_items_by_id($data['sales_info']->estimates_id);
            $data['sales_info']->sub_total = ($this->estimates_model->estimate_calculation('estimate_cost', $data['sales_info']->estimates_id));
            $data['sales_info']->discount = ($this->estimates_model->estimate_calculation('discount', $data['sales_info']->estimates_id));
            $data['sales_info']->total = ($this->estimates_model->estimate_calculation('total', $data['sales_info']->estimates_id));
            // get payment info by id
            $data['footer'] = config_item('estimate_footer');
        } else {
        ...
        $data['subview'] = $this->load->view('admin/estimates/estimates_details', $data, TRUE);
        $this->load->view('admin/_layout_main', $data); //page load
            ...
            foreach ($fetch_data as $_key => $v_estimates) {
                if (!empty($v_estimates)) {
                    ...
                    if (!empty($can_edit) && !empty($edited)) {
                        ...
                        $action .= btn_edit('admin/estimates/create/edit_estimates/' . $v_estimates->estimates_id) . ' ';
                    }
    ...
    {
        $this->estimates_details($id, true);
    }
    ...
    function save_estimates($id = NULL)
    {
        ...
        if (!empty($created) || !empty($edited) && !empty($id)) {
            ...
            $activity = array(
                ...
                'link' => 'admin/estimates/create/estimates_details/' . $estimates_id,
        ...
        if (!empty($data['project_id']) && is_numeric($data['project_id'])) {
            ...
            redirect('admin/estimates/create/estimates_details/' . $estimates_id);
        }
        redirect('admin/estimates');
    }
    ...
    function add_item($id = NULL)
    {
        ...
        if (!empty($can_edit) && !empty($edited)) {
            ...
            if (!empty($quantity)) {
                foreach ($quantity as $key => $value) {
                    ...
                    $activity = array(
                        ...
                        'activity' => $action,
                        'icon' => 'fa-shopping-cart',
                        'link' => 'admin/estimates/create/estimates_details/' . $data['estimates_id'],
            ...
            set_message($type, $message);
            redirect('admin/estimates/create/estimates_details/' . $data['estimates_id']);
        } else {
            set_message('error', lang('there_in_no_value'));
            if (empty($_SERVER['HTTP_REFERER'])) {
                redirect('admin/estimates');
    ...
    function cloned_estimate($id)
    {
        ...
        if (!empty($can_edit) && !empty($edited)) {
            ...
            $activities = array(
                ...
                'icon' => 'fa-shopping-cart',
                'link' => 'admin/estimates/create/estimates_details/' . $new_invoice_id,
            ...
            set_message($type, $message);
            redirect('admin/estimates/create/estimates_details/' . $new_invoice_id);
        } else {
            set_message('error', lang('there_in_no_value'));
            if (empty($_SERVER['HTTP_REFERER'])) {
                redirect('admin/estimates');
    ...
    function change_status($action, $id)
    {
        ...
        if (!empty($can_edit) && !empty($edited)) {
            ...
            set_message($type, $message);
            redirect('admin/estimates/create/estimates_details/' . $id);
        } else {
            set_message('error', lang('there_in_no_value'));
            if (empty($_SERVER['HTTP_REFERER'])) {
                redirect('admin/estimates');
    ...
    function send_estimates_email($estimates_id, $row = null)
    {
        ...
        $link = str_replace("{ESTIMATE_LINK}", base_url() . 'frontend/estimates/' . url_encode($estimates_id), $Currency);
    ...
    function send_email_estimates($estimates_id, $message, $subject)
    {
        ...
        $params['resourceed_file'] = 'uploads/' . slug_it(lang('estimates') . '_pdf_' . $estimates_info->reference_no) . '.pdf';
        $params['resourcement_url'] = base_url() . 'uploads/' . slug_it(lang('estimates') . '_pdf_' . $estimates_info->reference_no) . '.pdf';

        $this->attach_pdf($estimates_id);

        $this->estimates_model->send_email($params);
        //Delete estimate in tmp folder
        if (is_file('uploads/' . slug_it(lang('estimates') . '_pdf_' . $estimates_info->reference_no) . '.pdf')) {
            unlink('uploads/' . slug_it(lang('estimates') . '_pdf_' . $estimates_info->reference_no) . '.pdf');
        }
        // send notification to client
        if (!empty($client_info->primary_contact)) {
            $notifyUser = array($client_info->primary_contact);
    ...
    {
        $this->estimates_details($id, 'attach');
    }
    ...
    {
        $data = $this->estimates_model->array_from_post(array('reference_no', 'client_id', 'project_id', 'warehouse_id', 'discount_type', 'discount_percent', 'user_id', 'adjustment', 'discount_total', 'show_quantity_as'));
                    ...
                    if (!empty($qty_calculation) && $qty_calculation == 'Yes') {
                        ...
                        if (!empty($all_items_info)) {
                            foreach ($all_items_info as $v_items) {
                                $this->estimates_model->return_items($v_items->saved_items_id, $v_items->quantity, $data['warehouse_id']);
                            }
                        }
                    }
            ...
            foreach ($removed_items as $r_id) {
                if ($r_id != 'undefined') {
                    if (!empty($qty_calculation) && $qty_calculation == 'Yes') {
                        ...
                        $this->estimates_model->return_items($itemInfo->saved_items_id, $itemInfo->quantity, $data['warehouse_id']);
                    }
        ...
        $activity = array(
            ...
            'icon' => 'fa-shopping-cart',
            'link' => 'admin/estimates/create/estimates_details/' . $estimate_id,
            'value1' => $data['reference_no']
        );
        $this->estimates_model->_table_name = 'tbl_activities';
        $this->estimates_model->_primary_key = 'activities_id';
        ...
        set_message($type, $message);
        redirect('admin/estimates/create/estimates_details/' . $estimate_id);
    }
            ...
            if ($items_info->quantity != $qty) {
                if ($qty > $items_info->quantity) {
                    ...
                    if (!empty($items_info->saved_items_id)) {
                        $this->estimates_model->reduce_items($items_info->saved_items_id, $reduce_qty, $warehouseId);
                    }
                }
                if ($qty < $items_info->quantity) {
                    $return_qty = $items_info->quantity - $qty;
                    if (!empty($items_info->saved_items_id)) {
                        $this->estimates_model->return_items($items_info->saved_items_id, $return_qty, $warehouseId);
                    }
                }
            }
        }
