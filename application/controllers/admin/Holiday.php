...
class Holiday extends Admin_Controller
{
    ...
    public function index($flag = NULL, $id = NULL)
    {
        ...
        $this->common_model->_table_name = "tbl_holiday"; //table name
        $this->common_model->_order_by = "holiday_id";
        // get holiday list by id
        if (!empty($id)) {
            $data['holiday_list'] = $this->common_model->get_by(array('holiday_id' => $id,), TRUE);
    ...
    {// this function is to create get monthy recap report
        for ($i = 1; $i <= 12; $i++) { // query for months
            ...
            $get_holiday_list[$i] = $this->common_model->get_holiday_list_by_date($start_date, $end_date); // get all report by start date and in date
        }
    ...
    public function save_holiday($id = NULL)
    {
        ...
        if (!empty($created) || !empty($edited) && !empty($id)) {
            $this->common_model->_table_name = "tbl_holiday"; //table name
            $this->common_model->_primary_key = "holiday_id";    //id
            // input data
            $data = $this->common_model->array_from_post(array('event_name', 'description', 'start_date', 'end_date', 'location', 'color')); //input post
            ...
            $check_holiday = $this->common_model->check_update('tbl_holiday', $where, $holiday_id);
            ...
            if (!empty($check_holiday)) {
                ...
                $id = $this->common_model->save($data, $id);
                // messages for user
                $type = "success";
    ...
    { // delete holiday list by id
        ...
        if (!empty($deleted)) {
            $check_holiday = $this->common_model->check_by(array('holiday_id' => $id), 'tbl_holiday');
            $this->common_model->_table_name = "tbl_holiday"; //table name
            $this->common_model->_primary_key = "holiday_id";    //id
