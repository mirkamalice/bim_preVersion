...
class Bugs extends Admin_Controller
{
    ...
    public function bugs_state_report()
    {
        if (!$this->input->is_ajax_request()) {
            redirect('admin/dashboard');
        }
        $data = array();
        $pathonor_jonno['bugs_state_report_div'] = $this->load->view("admin/bugs/bugs_state_report", $data, true);
    ...
    public function create($id = null, $opt_id = NULL)
    {
        ...
        $data['active'] = 2;
        $data['subview'] = $this->load->view('admin/bugs/createbugs', $data, TRUE);
        $this->load->view('admin/_layout_main', $data);
    }
    ...
    public function bugsList($filterBy = null, $search_by = null)
    {
        if ($this->input->is_ajax_request()) {
            ...
            foreach ($fetch_data as $_key => $v_bugs) {
                if (!empty($v_bugs)) {
                    ...
                    $name = null;
                    $name .= '<a class="text-info" href="' . base_url() . 'admin/bugs/details/' . $v_bugs->bug_id . '">' . $v_bugs->bug_title . '</a>';
        ...
        <ul class="dropdown-menu animated zoomIn">';
                    ...
                    $sub_array[] = '<span class="tags label label-' . $label . '">' . lang($v_bugs->bug_status) . '</span>' . ' ' . $change_status;
                    ...
                    $sub_array[] = '<span class="tags label label-' . $plabel . '">' . lang($v_bugs->severity) . '</span>';

                    if ($this->session->userdata('user_type') == '1') {
                        $sub_array[] = '<span class="tags">' . fullname($v_bugs->reporter) . '</span>';
                    }
                    $assigned = null;
                    ...
                    if (!empty($can_edit) && !empty($edited)) {
                        $action .= btn_edit('admin/bugs/create/' . $v_bugs->bug_id) . ' ';
                    }
    ...
    public function save_bugs_notes($id = NULL)
    {
        ...
        $activities = array(
            ...
            'activity' => 'activity_update_bug',
            'icon' => 'fa-bug',
            'link' => 'admin/bugs/view_bug_details/' . $id . '/notes',
            'value1' => $data['notes'],
        );
        // Update into tbl_project
        $this->bugs_model->_table_name = "tbl_activities"; //table name
        ...
        $message = lang('update_bug');
        set_message($type, $message);
        redirect('admin/bugs/details/' . $id . '/notes');
    }
    ...
    public function delete_bug_files($attachments_id)
    {
        $file_info = $this->bugs_model->check_by(array('attachments_id' => $attachments_id), 'tbl_attachments');
        if (empty($file_info)) {
            echo json_encode(array("status" => 'error', 'message' => "No Record Found"));
            exit();
        }
        ...
        $this->bugs_model->delete_multiple(array('attachments_id' => $attachments_id));
    ...
    {
        $data['type'] = $type;
        $data['attachment_info'] = $this->bugs_model->check_by(array('attachments_id' => $id), 'tbl_attachments');
    ...
    public function save_attachment($attachments_id = NULL)
    {
        ...
        $this->bugs_model->_table_name = "tbl_attachments"; //table name
        $this->bugs_model->_primary_key = "attachments_id";
    ...
    public function download_files($uploaded_files_id, $comments = null)
    {
        ...
        if (!empty($comments)) {
            ...
            $uploaded_files_info = $this->bugs_model->check_by(array('uploaded_files_id' => $uploaded_files_id), 'tbl_attachments_files');
    ...
    public function save_bug_attachment($attachments_id = NULL)
    {
        $data = $this->bugs_model->array_from_post(array('title', 'description', 'bug_id'));
        $data['user_id'] = $this->session->userdata('user_id');
    ...
    function notify_attchemnt_bugs($attachments_id)
    {
        $email_template = email_templates(array('email_group' => 'bug_attachment'));
        $bugs_comment_info = $this->bugs_model->check_by(array('attachments_id' => $attachments_id), 'tbl_attachments');
    ...
    public function details($id, $active = NULL, $edit = NULL)
    {
        ...
        $data['all_tabs'] = bugs_details_tabs($id);
        $data['module'] = 'bugs';
        $data['id'] = $id;
    ...
    public function delete_bug($id)
    {
        ...
        if (!empty($can_delete) && !empty($deleted)) {
            ...
            $this->bugs_model->_primary_key = "activities_id";
            $this->bugs_model->save($activities);
            $mwhere = array('module' => 'bugs', 'module_field_id' => $id);
            $this->bugs_model->_table_name = "tbl_attachments"; //table name
            $this->bugs_model->_order_by = "module_field_id";
            $files_info = $this->bugs_model->get_by($mwhere, FALSE);
            ...
            foreach ($files_info as $v_files) {
                ...
                $this->bugs_model->_table_name = "tbl_attachments_files"; //table name
                $this->bugs_model->delete_multiple(array('attachments_id' => $v_files->attachments_id));
            }
            //delete into table.
            $this->bugs_model->_table_name = "tbl_attachments"; // table name
            $this->bugs_model->delete_multiple($mwhere);
    ...
    {
        $this->details($id, $active = NULL, $edit = NULL);
    }
}