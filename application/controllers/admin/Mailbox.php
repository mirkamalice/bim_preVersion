...
class Mailbox extends Admin_Controller
{
    ...
    public function index($action = NULL, $id = NULL, $status = NULL)
    {
        ...
        if (count(array($unread_mail)) > 0) {
            $unread_mail = count(array($unread_mail));
        } else {
            $unread_mail = 0;
        }
    ...
    public function send_mail()
    {
        ...
        // check $myemail is not exist in $all_email array
        if (in_array($myemail, $all_email)) {
            $key = array_search($myemail, $all_email);
            unset($all_email[$key]);
        }
        // remove
