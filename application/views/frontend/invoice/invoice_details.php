...
<div class="row mb">
    ...
    <div class="col-sm-8">
            ...
            <div class="btn-group">
                ...
                <ul class="dropdown-menu animated zoomIn">
                    ...
                    foreach ($all_payment as $key => $payment) {
                        $status = $this->config->item(slug_it(strtolower($payment->gateway_name)) . '_status');                       
                        $allow_gateway = 'allow_' . slug_it(strtolower($payment->gateway_name));
                        if ($status == 'active' && $invoice_info->$allow_gateway == 'Yes') {
                            $modal = '';
                            if ($payment->modal == 'Yes') {
                                $modal = 'data-toggle="modal" data-target="#myModal"';
                            }
                    ?>
                            <li><a <?= $modal ?> href="<?= base_url($payment->link . '/pay' . '/' . $invoice_info->invoices_id) ?>" title="<?= lang($payment->gateway_name) ?>"><?= lang($payment->gateway_name) ?></a></li>
                    <?php }
                    } ?>
                </ul>
            </div>
        ...
        ?>
            ...
            <a href="<?= base_url() ?>client/project/project_details/<?= $invoice_info->project_id ?>" class="">
    ...
    <div class="col-sm-4 pull-right">
        ...
        <a href="<?= base_url() ?>frontend/pdf_invoice/<?= $invoice_info->invoices_id ?>" data-toggle="tooltip" data-placement="top" title="" data-original-title="PDF" class="btn btn-sm btn-success pull-right mr-sm">
        ...
        <div class="row">
            ...
            <div class="col-lg-6 col-xs-12 ">
                ...
                <div class="pull-right pr-lg">
                    ...
                    ?>
                    <br><?= $language_info['payment_status'] ?>: <span class="label label-<?= $label ?>"><?= $payment_status ?></span>
                    <?php if (!empty($text)) { ?>
                        <br>
