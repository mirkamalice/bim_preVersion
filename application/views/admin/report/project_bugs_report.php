...
<div class="row">
    ...
    <div class="col-lg-6">
        ...
        <div id="panelChart4" class="panel panel-custom">
            <div class="panel-heading">
                <div class="panel-title"><?= lang('bugs') . ' ' . lang('report') . ' ' . jdate('Y') ?></div>
            </div>
            <div class="panel-body">
    ...
    foreach ($all_project as $v_project) :
        ...
        if (!empty($bugs_info)):foreach ($bugs_info as $v_bugs):
            if ($v_bugs->bug_status == 'unconfirmed') {
                $unconfirmed += count((array)$v_bugs->bug_status);
            }
            if ($v_bugs->bug_status == 'in_progress') {
                $in_progress += count((array)$v_bugs->bug_status);
            }
            if ($v_bugs->bug_status == 'confirmed') {
                $confirmed += count((array)$v_bugs->bug_status);
            }
            if ($v_bugs->bug_status == 'resolved') {
                $resolved += count((array)$v_bugs->bug_status);
            }
            if ($v_bugs->bug_status == 'verified') {
                $verified += count((array)$v_bugs->bug_status);
            }
        endforeach;
        endif;
    endforeach;
                ...
                ?>
                {
                    ...
                    foreach ($v_bugs_user as $status => $value) {
                        if ($status == 'unconfirmed') {
                            $aunconfirmed = count((array)$value);
                        } elseif ($status == 'in_progress') {
                            $inparogress = count((array)$value);
                        } elseif ($status == 'verified') {
                            $averified = count((array)$value);
                        } elseif ($status == 'resolved') {
                            $aresolved = count((array)$value);
                        }
                    }
                    ?>
                    a:<?= $aunconfirmed;?> ,
                        ...
                        ["<?= $month_name?>", <?php
                            ...
                            foreach ($v_report as $s_report) {
                                if ($s_report->bug_status == 'unconfirmed')
                                    $y_not_started += count((array)$s_report->bug_status);
                            }
