...
<header class="hidden-print">
    <div class="row ">
        <div class="col-sm-12">
            ...
            <a href="<?= base_url() ?>admin/tickets/create/edit_tickets/<?= $tickets_info->tickets_id ?>"
            ...
            <a data-toggle="tooltip" data-placement="top" title="<?= $title ?>"
                href="<?= base_url() ?>admin/projects/<?= $url ?>" class="mr-sm btn btn-xs btn-<?= $btn ?>"><i
        ...
        <ul class="list-group no-radius">
            ...
            <li class="list-group-item">
                ...
                <span class="pull-right">
                    <a href="#" class="recect_task pull-left">
                        <?php
                        ...
                        <img style="width: 18px;margin-left: 18px;
                            ...
                            src="<?= base_url() . staffImage($tickets_info->reporter) ?>" class="img-circle">
                        <?=
            ...
            ?>
            <li class="list-group-item">
                <span class="pull-right"><label class="label label-<?= $s_label ?>"><?= lang($status) ?></label>
                </span><?= lang('status') ?>
            </li>
            <li class="list-group-item"><span
                    class="pull-right"><?= get_tags($tickets_info->tags, true) ?></span><?= lang('tags') ?></li>
            <li class="list-group-item"><span
            ...
            if (!empty($show_custom_fields)) {
                foreach ($show_custom_fields as $c_label => $v_fields) {
                    if (!empty($v_fields)) {
            ?>
            <li class="list-group-item"><span class="pull-right"><?= $v_fields ?></span><?= $c_label ?></li>
            <?php }
                }
            }
            ?>
                            ...
                            if (!empty($v_files)) :
                                ...
                                <li>
                                    ...
                                    <div class="mailbox-attachment-info">
                                        <a href="<?= base_url() ?>admin/tickets/download_file/<?= $tickets_info->tickets_id . '/' . $fKey ?>"
                    ...
                    $("#<?php echo $comment_type; ?>-comment-form").appForm({
                        ...
                        onSuccess: function(result) {
                            ...
                            $(result.data).insertAfter(
                                "#<?php echo $comment_type; ?>-comment-form");
                                ...
                                success: function(response) {
                                    if (response.success) {
                                        ...
                                        $(file.previewTemplate).find(
                                            ".description-field").attr("name",
                                            "comment_" + fileSerial);
                                        $(file.previewTemplate).append(
                                            "<input type='hidden' name='file_name_" +
                                            fileSerial + "' value='" + file.name + "' />\n\
                                                                        <input type='hidden' name='file_size_" +
                                            fileSerial + "' value='" + file.size +
                                            "' />");
                                        $(file.previewTemplate).find(
                                            ".file-count-field").val(fileSerial);
                                        done();
                                    } else {
...
$(document).ready(function() {
    ...
    $('#tab_collapse').on("click", function() {
