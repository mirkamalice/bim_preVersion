...
?>
            <div class="col-sm-12 mb-mails" id="ticket-reply-container-<?php echo $v_replies->tickets_replies_id ?>">
                ...
                <div class="mb-mail-meta">
                    <div class="pull-left">
                        <div class="mb-mail-from"><a href="<?= base_url() ?>admin/user/user_details/<?= $v_replies->replierid ?>"> <?= ($profile_info->fullname) . ' ' . $label ?></a>
                        </div>
                    </div>
                    ...
                    <div class="mb-mail-album pull-left">
                        ...
                        if (!empty($uploaded_file)) {
                            foreach ($uploaded_file as $fKey => $v_files) {
                                if (!empty($v_files)) {
                        ?>
                                    <a href="<?= base_url() ?>admin/tickets/download_file/<?= $v_replies->tickets_id . '/' . $fKey . '/' . $v_replies->tickets_replies_id ?>">
                                        <?php if ($v_files->is_image == 1) { ?>
                                            <img alt="" src="<?= base_url() . $v_files->path ?>">
                                        <?php } else { ?>
                                            <div class="mail-attachment-info">
                                                <a href="<?= base_url() ?>admin/tickets/download_file/<?= $v_replies->tickets_id . '/' . $fKey . '/' . $v_replies->tickets_replies_id ?>" class="mail-attachment-name"><i class="fa fa-paperclip"></i> <?= $v_files->size ?> <?= lang('kb') ?>
                                                </a>

                                                <a href="<?= base_url() ?>admin/tickets/download_file/<?= $v_replies->tickets_id . '/' . $fKey . '/' . $v_replies->tickets_replies_id ?>" class="btn btn-default btn-xs pull-right"><i class="fa fa-cloud-download"></i></a>
                    ...
                    <button class="text-primary reply" data-toggle="tooltip" data-placement="top" title="<?= lang('click_to_reply') ?>" id="reply__<?php echo $v_replies->tickets_replies_id ?>"><i class="fa fa-reply "></i> <?= lang('reply') ?>
                    </button>
                    ...
                    <div class="reply__" id="reply_<?php echo $v_replies->tickets_replies_id ?>">
                        <form id="<?php echo $comment_reply_type; ?>-comment-form-<?php echo $v_replies->tickets_replies_id ?>" action="<?php echo base_url() ?>admin/tickets/save_comments_reply/<?php
                            ...
                            <button type="submit" id="reply-<?php echo $v_replies->tickets_replies_id ?>" class="btn btn-xs btn-primary"><?= lang('save') ?></button>
