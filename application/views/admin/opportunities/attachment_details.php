...
if ($type == 'g') {
    $v_files_info = $this->db->where('attachments_id', $attachment_info->attachments_id)->get('tbl_attachments_files')->result();
    $height = '450px';
    $title = $attachment_info->title;
    $comment_details = $this->db->where(array('attachments_id' => $attachment_info->attachments_id, 'comments_reply_id' => '0'))->order_by('comment_datetime', 'DESC')->get('tbl_task_comment')->result();
    $description = $attachment_info->description;
} else {
    $uploaded_files_id = $this->uri->segment(6);
    $v_files_info = $this->db->where('uploaded_files_id', $uploaded_files_id)->get('tbl_attachments_files')->result();
    $height = '600px';
    ...
    <div class="panel-heading">
        ...
        <h4 class="panel-title" id="myModalLabel"><?= $title . '-' . lang('details') ?>
            <a data-toggle="tooltip" data-placement="top"
               ...
               href="<?= base_url() ?>admin/opportunities/download_all_files/<?= $attachment_info->attachments_id ?>"
               class="pull-right"><i
                    ...
                    <div class="panel panel-custom">
                        ...
                        <div class="panel-body chat" id="chat-box">
                            ...
                            <?php if ($type == 'g') { ?>
                                ...
                                if (!empty($attachment_info->attachments_id)) {
                                    echo $attachment_info->attachments_id;
                                }
                                ?>" class="form-control">
                            <?php } else { ?>
