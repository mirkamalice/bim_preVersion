...
class Settings_Model extends MY_Model
{
        ...
        foreach ($languages as $lang) {
            ...
            foreach ($files as $file => $altpath) {
                ...
                if ($lang != 'english') {
                    ...
                    $diff = $count = count(array($en));
                    $fstats[$shortfile] = array(
                        "total" => count(array($en)),
                        "translated" => $diff,
                    );
                }
                $total += $count;
    ...
    function add_language($language, $files)
    {
        ...
        $lang = $this->db->get_where('tbl_locales', array('name' => str_replace("_", " ", $language)))->row();
        $l = $lang;
        $slug = slug_it($language);
    ...
    function save_translation($post = array())
    {
        ...
        //        $eng = $this->lang->load($file, 'english', TRUE, TRUE, $fullpath);
        //
        //        if ($language == 'english') {
        //            $trn = $eng;
        //        } else {
