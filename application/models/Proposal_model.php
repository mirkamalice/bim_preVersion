...
class Proposal_Model extends MY_Model
{
    ...
    function ordered_items_by_id($id, $json = null)
    {
        ...
        if (!empty($json)) {
            if (!empty($rows)) {
                foreach ($rows as $row) {
                    $row->items_id = $row->proposals_items_id;
    ...
    public function get_invoice_filter()
    {
        ...
        $this->db->select('proposal_year');
        $this->db->group_by('proposal_year');
        $result = $this->db->get('tbl_proposals')->result();
        ...
        if (!empty($result)) {
            foreach ($result as $v_year) {
                $test = array(
                    'id' => 1,
                    'value' => '_' . $v_year->proposal_year,
                    'name' => $v_year->proposal_year,
                    'order' => 1
                );
                if (!empty($test)) {
