...
class CurlHandleTest extends \Guzzle\Tests\GuzzleTestCase
{
    ...
    public function testConstructorExpectsProperOptions()
    {
       ...
       $this->assertEquals($this->getServer()->getUrl(), $ha->config_items()->get(CURLOPT_URL));
    ...
    public function testConstructorInitializesObject()
    {
        ...
        $this->assertEquals($this->getServer()->getUrl(), $h->config_items()->get(CURLOPT_URL));
    }
    ...
    public function testStoresStdErr()
    {
        ...
        $this->assertEquals($h->getStderr(true), $h->config_items()->get(CURLOPT_STDERR));
    ...
    public function testWrapsCurlOptions()
    {
        ...
        $this->assertEquals(true, $h->config_items()->get(CURLOPT_AUTOREFERER));
        $this->assertEquals(1024, $h->config_items()->get(CURLOPT_BUFFERSIZE));
    }
    ...
    public function testSendsPostRequestsWithFiles()
    {
        ...
        $options = $this->requestHandle->config_items()->getAll();
        if (version_compare(phpversion(), '5.5.0', '<')) {
    ...
    public function testCurlConfigurationOptionsAreSet()
    {
        ...
        $handle = CurlHandle::factory($request);
        $this->assertEquals(99, $handle->config_items()->get(CURLOPT_CONNECTTIMEOUT));
        $this->assertEquals(8181, $handle->config_items()->get(CURLOPT_PORT));
        $this->assertNull($handle->config_items()->get('curl.fake_opt'));
        $this->assertNull($handle->config_items()->get('fake_opt'));
    }
    ...
    public function testAllowsWireTransferInfoToBeEnabled()
    {
        ...
        $request->getCurlOptions()->set('debug', true);
        $handle = CurlHandle::factory($request);
        $this->assertNotNull($handle->config_items()->get(CURLOPT_STDERR));
        $this->assertNotNull($handle->config_items()->get(CURLOPT_VERBOSE));
    }
    ...
    public function testAddsCustomCurlOptions()
    {
        ...
        $handle = CurlHandle::factory($request);
        $this->assertEquals(200, $handle->config_items()->get(CURLOPT_TIMEOUT));
    }
