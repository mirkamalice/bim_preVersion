<script language="javascript" type="text/javascript">
    ...
    function check_old_password(val) {
        ...
        var strURL = base_url + "admin/common/check_current_password/" + val;
        var req = getXMLHTTP();
        if (req) {
    ...
    function get_milestone_by_id(project_id) {
        var base_url = '<?= base_url() ?>';
        var strURL = base_url + "admin/common/get_milestone_by_project_id/" + project_id;
        var req = getXMLHTTP();
        if (req) {
            req.onreadystatechange = function () {
                if (req.readyState == 4) {
    ...
    function check_user_name(str) {
        ...
        var base_url = '<?= base_url() ?>';
        var strURL = base_url + "admin/common/check_existing_user_name/" + user_name + "/" + user_id;
        var req = getXMLHTTP();
        if (req) {
            req.onreadystatechange = function () {
                if (req.readyState == 4) {
    ...
    function get_item_name_by_id(stock_sub_category_id) {
        var base_url = '<?= base_url() ?>';
        var strURL = base_url + "admin/common/get_item_name_by_id/" + stock_sub_category_id;
        var req = getXMLHTTP();
        if (req) {
            req.onreadystatechange = function () {
                if (req.readyState == 4) {
    ...
    function check_advance_amount(str) {
        ...
        var user_id = $.trim($("#user_id").val());
        var base_url = '<?= base_url() ?>';
        var strURL = base_url + "admin/common/check_advance_amount/" + amount + "/" + user_id;
        var req = getXMLHTTP();
        if (req) {
            req.onreadystatechange = function () {
                if (req.readyState == 4) {
    ...
    function get_leave_details(user_id) {
        if (user_id) {
            var strURL = base_url + "admin/common/get_leave_details/" + user_id;
    ...
    function check_available_leave(date) {
        ...
        var base_url = '<?= base_url() ?>';
        var strURL = base_url + "admin/common/check_available_leave/" + user_id + "/" + start_date + "/" + end_date + "/" + leave_category_id;
        var req = getXMLHTTP();
        if (req) {
            req.onreadystatechange = function () {
                if (req.readyState == 4) {
    ...
    function get_employee_by_designations_id(designation_id) {
        var base_url = '<?= base_url() ?>';
        var strURL = base_url + "admin/common/get_employee_by_designations_id/" + designation_id;
        var req = getXMLHTTP();
        if (req) {
            req.onreadystatechange = function () {
                if (req.readyState == 4) {
    ...
    function check_duplicate_emp_id(val) {
        var base_url = '<?= base_url() ?>';
        var strURL = base_url + "admin/common/check_duplicate_emp_id/" + val;
        var req = getXMLHTTP();
        if (req) {
            req.onreadystatechange = function () {
                if (req.readyState == 4) {
    ...
    function check_match_password(val) {
        ...
        var strURL = base_url + "admin/common/check_match_password/" + val + "/" + password;
        var req = getXMLHTTP();
        if (req) {
            req.onreadystatechange = function () {
                if (req.readyState == 4) {
